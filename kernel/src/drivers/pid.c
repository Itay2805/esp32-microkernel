...
pid_binding_t g_pid_bindings[PID_BINDING_COUNT] = {
    { .pid = 2 },
    { .pid = 3 },
    { .pid = 4 },
    { .pid = 5 },
...
void init_pid() {
    // set the vecbase
    __WSR(VECBASE, g_vdso_header.vecbase);

    // we are going to skip the kernel exception vector since we care about it less
    PIDCTRL_INTERRUPT_ADDR[0] = g_vdso_header.vecbase + 0x340; /* user exception handler */
    PIDCTRL_INTERRUPT_ADDR[1] = g_vdso_header.vecbase + 0x180; /* interrupt level 2 */
    PIDCTRL_INTERRUPT_ADDR[2] = g_vdso_header.vecbase + 0x1c0; /* interrupt level 3 */
    PIDCTRL_INTERRUPT_ADDR[3] = g_vdso_header.vecbase + 0x200; /* interrupt level 4 */
    PIDCTRL_INTERRUPT_ADDR[4] = g_vdso_header.vecbase + 0x240; /* interrupt level 5 */
    PIDCTRL_INTERRUPT_ADDR[5] = g_vdso_header.vecbase + 0x280; /* interrupt level 6 (debug) */
    PIDCTRL_INTERRUPT_ADDR[6] = g_vdso_header.vecbase + 0x2c0; /* interrupt level 7 (nmi) */
